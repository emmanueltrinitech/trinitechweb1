            ...
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'webserver_login', usernameVariable: 'USERNAME', passwordVariable: 'USERPASS')]) {
                    script {
                        ...
                        sh "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no $USERNAME@$prod_ip \"docker run -tid -p 6222:22 -p 6083:80 --name=website2 --tmpfs /run -v /sys/fs/cgroup:/sys/fs/cgroup:ro  --restart always ewarah/website2:${env.BUILD_NUMBER} /bin/bash\""
                    }
                }
            }
        }
